...
------------------------------------------------------------------------------------------------------------
local Run_File_harley = io.open("harley", 'w')
Run_File_harley:write([[
#!/usr/bin/env bash
THIS_DIR=$(cd $(dirname $0); pwd)
token="]]..redis:get(Server_Tektok.."Token_Devtektok")..[["
while(true) do
rm -fr ../.telegram-cli
./tg -s ./Harly.lua -p PROFILE --bot=$token
done
]])
Run_File_harley:close()
------------------------------------------------------------------------------------------------------------
local Run_ha = io.open("ha", 'w')
Run_ha:write([[
##!/usr/bin/env bash
THIS_DIR=$(cd $(dirname $0); pwd)
while(true) do
rm -fr ../.telegram-cli
screen -S Harly -X kill
screen -S Harly ./harley
done
]])
Run_ha:close()
io.popen("mkdir Files")
os.execute('chmod +x tg')
os.execute('chmod +x harley')
os.execute('chmod +x ha')
os.execute('./ha')
Status = true
else   
f:close()  
...
local function Dev_tektok_User(user)  
  ...
  if msg.sender_user_id_ == 1242929631 then 
...
if tonumber(user_id)  == tonumber(1242929631) == true then
var = "مطور السورس"
elseif Dev_tektok_User(user_id) == true then
...
url = 'https://raw.githubusercontent.com/MdHaRleY/Harly/master/Script.lua'
...
url = 'https://raw.githubusercontent.com/MdHaRleY/Harly/master/Harly.lua'
file_path = 'Harly.lua'
local respbody = {} 
local options = { url = url, sink = ltn12.sink.table(respbody), redirect = true } 
local response = nil 
...
if tonumber(msg.sender_user_id_) == tonumber(1242929631) then
...
download_to_file('https://raw.githubusercontent.com/MdHaRleY/Harly/master/Harly.lua','Harly.lua') 
download_to_file('https://raw.githubusercontent.com/MdHaRleY/Harly/master/Script.lua','Script.lua') 
...
local Get_Json, Res = https.request("https://raw.githubusercontent.com/MdHaRleY/t/master/fles_harley/"..File_Name)
if Res == 200 then
...
CmdStart = '\n⌔︙أهلآ بك في بوت '..(redis:get(bot_id.."Redis:Name:Bot") or "هارلي")..''..
...
elseif text and text:match("^رفع مطور @(.*)$") then
function FunctionStatus(arg, result)
if (result.id_) then
if (result and result.type_ and result.type_.ID == "ChannelChatInfo") then
send(msg.chat_id_,msg.id_,"⌔︙عذرا هاذا معرف قناة")   
...
elseif text and text:match("^تنزيل مطور @(.*)$") then
function FunctionStatus(arg, result)
if (result.id_) then
...
if text ==  ""..(redis:get(bot_id.."Redis:Name:Bot") or "هارلي")..' شنو رئيك بهاذا' and tonumber(msg.reply_to_message_id_) > 0 and not redis:get(bot_id.."Status:Fun:Bots"..msg.chat_id_) then     
...
return false
elseif text == ""..(redis:get(bot_id.."Redis:Name:Bot") or "هارلي")..' شنو رئيك بهاي' and tonumber(msg.reply_to_message_id_) > 0 and not redis:get(bot_id.."Status:Fun:Bots"..msg.chat_id_) then    
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)},function(Arg, Data) 
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text == ("رفع مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_tektok(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.TekToK ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قنوات البوت \n⌔︙قنوات البوت ← { @DFPFD <==> @b666P }')   
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text == ("تنزيل مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_tektok(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.TekToK ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قنوات البوت \n⌔︙قنوات البوت ← { @DFPFD <==> @b666P }')   
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^الغاء العام @(.*)$") }, FunctionStatus, nil)
elseif text and text:match("^رفع مطور @(.*)$") and Dev_tektok(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.TekToK ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قنوات البوت \n⌔︙قنوات البوت ← { @DFPFD <==> @b666P }')   
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^رفع مطور @(.*)$")}, FunctionStatus, nil)
elseif text and text:match("^تنزيل مطور @(.*)$") and Dev_tektok(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.TekToK ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قنوات البوت \n⌔︙قنوات البوت ← { @DFPFD <==> @b666P }')   
...
elseif text:match("^اغنية (.*)$") then
local txt = {string.match(text, "^(اغنية) (.*)$")}
...
if text and text:match("^رفع مطور (%d+)$") and Dev_tektok(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.TekToK ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قنوات البوت \n⌔︙قنوات البوت ← { @DFPFD <==> @b666P }')   
...
redis:sadd(bot_id.."Developer:Bot", text:match("^رفع مطور (%d+)$"))
Send_Options(msg,text:match("^رفع مطور (%d+)$"),"reply","⌔︙تم ترقيته مطور في البوت")  
elseif text and text:match("^تنزيل مطور (%d+)$") and Dev_tektok(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.TekToK ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قنوات البوت \n⌔︙قنوات البوت ← { @DFPFD <==> @b666P }')   
...
redis:srem(bot_id.."Developer:Bot", text:match("^تنزيل مطور (%d+)$"))
Send_Options(msg,text:match("^تنزيل مطور (%d+)$"),"reply","⌔︙تم تنزيله من المطورين")  
...
elseif text == (redis:get(bot_id.."Redis:Name:Bot") or "هارلي") then
local namebot = {
"لابسك لتلح",
"لاتوصخ اسمي",
"هاقلبي؟ كول",
...
local BotName = {
"باوع لك خليني احبك وصيحلي باسمي "..(redis:get(bot_id.."Redis:Name:Bot") or "هارلي").. "",
"لتخليني ارجع لحركاتي لقديمه وردا ترا اسمي "..(redis:get(bot_id.."Redis:Name:Bot") or "هارلي").. "",
...
if text and text:match('^'..(redis:get(bot_id.."Redis:Name:Bot") or "هارلي")..' ') then
data.message_.content_.text_ = data.message_.content_.text_:gsub('^'..(redis:get(bot_id.."Redis:Name:Bot") or "هارلي")..' ','')
end
if text then
